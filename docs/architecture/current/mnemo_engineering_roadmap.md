...
**Architectural Input:** Mnemo Architecture Specification v2.0 (living current architecture)  
...
**Current baseline (2026-09-08):** Phases 0–8 are complete. Phase 8.5 is
complete and certified under ADR-0076 for the exact immutable 44-document V2
production identity. Phase 8.6 is complete as a validated 24-document,
5,843-chunk format-diverse multilingual evaluation notebook and is not
...
Phase 0  →  Phase 1  →  Phase 2  →  Phase 3  →  Phase 4
   ...
   Env         Scaff.       Layer         System        Engine
     ...
     ↓             ↓             ↓             ↓
Phase 5  →  Phase 6  →  Phase 7  →  Phase 8  →  Phase 8.5  →  Phase 8.6
  Embed.       Retrieval     REST API      MCP          Advanced       Evaluation
  Pipeline     Pipeline      + Streaming   Server       retrieval/MM   notebook

                                      Phase 8.7  →  Phase 8.8  →  Phase 9
                                      MCP/client    Convergence   Web UI
                                      capability    + contracts   (next)
...
**Duration:** Weeks 4–6  
**Status:** Complete
**Goal:** Establish the four-adapter storage framework. Filesystem and SQLite
provide the complete current authority; Qdrant provides a functional optional
V1 vector adapter; SurrealDB provides the partial graph-specific adapter. This
phase does not establish a requirement that every indexed file exist in all four
stores.
...
**Goal:** Complete retrieval pipeline operational — provider-backed dense
retrieval, SQLite FTS5 sparse retrieval, query expansion, parent retrieval,
cross-encoder reranking, RRF fusion, context assembly, and citations. Historical
V1 dense milestones used Qdrant; certified V2 uses immutable SQLite BGE-M3
vectors and bounded in-process exact cosine retrieval.
...
### Phase 8.5 — Advanced Retrieval and Multimodal Foundation

**Status:** **COMPLETED / CERTIFIED** under ADR-0076
**Goal:** Add original/occurrence asset provenance, durable optional processing,
advanced and structured retrieval with explicit completeness, multimodal and
multilingual evidence, bounded HTTP/MCP resource delivery, and Final QA V2
without changing frozen Phase 0–8 contracts.

The normative execution plan is the
[Phase 8.5 Complete Implementation Plan](../../governance/historical/PHASE_8_5_COMPLETE_IMPLEMENTATION_PLAN.md),
with the dedicated historical roadmap retained as evidence and ADR-0058 through
ADR-0076 governing decisions. The additive 8.5.1 asset foundation, 8.5.2
bounded parser asset extraction, 8.5.3 provider-neutral durable processing and
governance, 8.5.4 governed immutable OCR derivations, 8.5.5 governed
vision-analysis and visual-vector derivations, and 8.5.6 bounded
ranked/exhaustive retrieval with truthful completeness and signed snapshot
cursors, 8.5.7 deterministic structured retrieval/aggregation over
exact-version table projections, 8.5.8 typed multimodal fusion/context,
8.5.9 provider-neutral multilingual provenance/retrieval, and 8.5.10 bounded
HTTP/MCP V2 resource delivery are implemented and tested. Evidence citations
and immutable Final-QA V2 are implemented. Production-parity evaluation,
durable BGE activation/rollback, active HTTP/MCP verification, controlled WP-16
evidence, and WP-17 lifecycle certification are complete for the bound
production identity. This does not certify the Phase 8.6 evaluation notebook or
every optional structured/multimodal generation.

---

### Phase 8.6 — Format-Diverse Multilingual Evaluation Notebook

**Status:** **COMPLETED / VALIDATED / EVALUATION-ONLY**
**Goal:** Validate native-format ingestion and retrieval across governed English,
Hindi, and Marathi PDF, HTML, DOCX, PPTX, XLSX, CSV, JSON, and Markdown sources
without changing the production corpus.

The retained READY notebook contains 24 documents, 5,843 chunks/FTS rows/BGE-M3
embeddings, and 161 image occurrences with 161 OCR, Vision, and CLIP derivations.
Oversized PDF/XLSX table rows and Markdown lists/tables use deterministic
structure-aware subdivision. Canonical UTF-8 manifest identity, registry binding,
and HTTP/MCP stdio/MCP SSE parity are validated. The notebook remains isolated
and evaluation-only.

---

### Phase 8.7 — MCP and Client Capability Expansion

**Status:** **COMPLETED CAPABILITY MILESTONE; PHASE 8.8 HARDENING REQUIRED**
**Goal:** Expand MCP beyond the original six tools with native evidence,
structured, delivery, capability, and FinalQA operations and exercise them from
real clients.

The repository now registers 14 MCP tools. Real client work proved discovery and
several delivery/V2 paths, then exposed a historical tunnel startup profile,
immutable-schema compatibility failures in retained readers, incomplete central
authorization for retained tunnel tools, opaque source metadata, and capability/
error ambiguity. Phase 8.7 remains the implementation milestone; Phase 8.8 owns
the discovered correction and certification work.
...
**Status:** **NEXT IMPLEMENTATION PHASE — BLOCKED UNTIL PHASE 8.8 VERIFIED**  
...
**Module 2.4 — SurrealDB Store (Historical Plan; Partial Adapter)**

> **Historical Phase 2 adapter plan, not current certified persistence:** the
> checklist below records the original intended SurrealDB surface and early
> adapter work. The current adapter is partial. Certified V2 stores canonical
> source/asset bytes in the content-addressed filesystem, immutable corpus and
> retrieval state in SQLite, and mutable FinalQA execution state in a separate
> operational SQLite store. SurrealDB graph persistence/retrieval remains future
> Phase 11 work and is not a Phase 8.8 or Phase 9 prerequisite.
...
| Task | Subtask | Notes | Difficulty | Dependency |
|---|---|---|---|---|
| ✅ Implement `CompositeStorage` | Routes method calls to enabled adapters | Filesystem/SQLite are complete; Qdrant is optional V1; graph operations may route to the partial SurrealDB adapter | High | 2.1–2.4 |
...
> **Historical / evaluation-only V1 acceptance evidence:** Complete and locally
> validated under accepted ADR-0042. Module
...
> milestone validation has started. This MS-Marco evidence is retained as
> history; it is not the current configured or certified production reranker.
> Certified V2 uses `BAAI/bge-reranker-v2-m3` revision
> `953dc6f6f85a1b2dbfca4c34a2796e7dde08d41e` under the governed
> `bge-reranker-v2-m3-pair-256-contextual-v1` policy, CUDA batch 2, with CPU
> fallback forbidden.

| Task | Notes | Ref. Repo | Difficulty | Dependency |
|---|---|---|---|---|
| ✅ Implement `CrossEncoderReranker` | **Historical/evaluation-only V1:** ADR-0042 additive fusion-aware contract with pinned sentence-transformers MS MARCO MiniLM L6 reference provider; not the certified V2 production reranker | RAGFlow | High | 6.5 |
...
> `docs/reports/audits/audit_report_phase0-8-final.md`.
...
Implementation is decomposed into twenty workstreams and milestones M0–M12 in
the [dedicated roadmap](../historical/mnemo_phase8_5_engineering_roadmap.md). This master
roadmap intentionally does not duplicate that historical task matrix. Entry
required the v0.25.0 baseline; exit evidence now includes the complete asset,
job, extraction, retrieval, FinalQA V2, multilingual, HTTP/MCP, security,
evaluation, activation/rollback, lifecycle, documentation, and CI gates. The
current certification is governed by ADR-0076.

---

### Phase 8.6 — Format-Diverse Multilingual Evaluation Notebook

**Status:** Completed and validated; evaluation-only

| Task | Evidence-backed result | Dependency |
|---|---|---|
| Govern multilingual/native-format corpus | 24 source identities spanning PDF, HTML, DOCX, PPTX, XLSX, CSV, JSON, and Markdown in English, Hindi, and Marathi | Phase 8.5 |
| Ingest and chunk | 5,843 chunks; oversized atomic structures subdivided deterministically without persisted over-limit chunks | 8.6a |
| Build text and multimodal projections | 5,843 FTS/BGE-M3 embeddings; 161 image occurrences with complete OCR, Vision, and CLIP derivations | 8.6b |
| Bind canonical manifest | Shared UTF-8 canonical JSON digest; store identity and registry validated without data regeneration | 8.6c |
| Validate transports | HTTP, MCP stdio, and MCP SSE resolve the same evaluation identity and results | 8.6d |
| Preserve isolation | READY in the evaluation registry; not production-exposed and not merged into the 44-document corpus | 8.6e |

---

### Phase 8.7 — MCP and Client Capability Expansion

**Status:** Completed capability milestone; correction work assigned to Phase 8.8

| Task | Evidence-backed result | Dependency |
|---|---|---|
| Expand MCP surface | 14 registered tools: six retained, four delivery, and four V2 evidence/capability/FinalQA tools | Phase 8.6 |
| Preserve stable identities | Notebook/source/document/version/chunk/asset/occurrence identities carried through additive contracts | 8.7a |
| Exercise real clients | Tool discovery, retrieval, delivery, analysis, and FinalQA paths exercised through native and external clients | 8.7a |
| Record discovered gaps | Tunnel configuration drift, retained-reader incompatibility, retained-tool authorization, metadata, capability, error, and parity gaps captured by the MCP audits | 8.7b |

This retrospective phase label does not claim that the tunnel or all registered
tools were production-certified. It preserves the implementation chronology and
makes Phase 8.8 the corrective hardening phase.
...
| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Define one production startup binding | HTTP, stdio, SSE, and existing tunnel resolve the canonical config, immutable DB identity, model revisions, durable activation, authorization, FinalQA operational store, and certification digest | High | Phase 8.7 |
| Reject configuration forks | Fail before tool exposure when any effective identity differs | High | 8.8.1a |
| Bind tunnel to server-owned startup | Tunnel selects a transport, never a historical database/model graph | Medium | 8.8.1a |
| Capture convergence evidence | Record effective identity and configuration digest per transport | Medium | 8.8.1b |

**Module 8.8.2 — Immutable Certified Schema-Compatible Readers**

**Mandatory defect closure — `sq-2:sparse` / missing page-position columns**

This module must explicitly close the confirmed Mnemo reader defect in which
FTS5 successfully returns candidates from the certified immutable corpus, but
...
The required correction is a shared immutable-schema-compatible read model for
every retained path that reads chunks, including sparse retrieval,
canonical-text evidence, block retrieval, and exact-chunk delivery. It must
select only columns physically present in the governed corpus schema, preserve
stable document/version/chunk identities and provenance, and represent absent
...
| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Reproduce and pin the confirmed failure | Exercise the certified immutable schema and prove the existing chain: FTS5 candidate success -> `SQLiteStore.get_chunk()` -> missing `position_page_start/end` -> `OperationalError` -> opaque `sq-2:sparse` wrapper | Medium | 8.8.1 |
| Introduce one compatible read-model boundary | Serve retained sparse, canonical evidence, block, and exact-chunk reads without assuming `chunks.position_page_start` or `chunks.position_page_end` exists | High | 8.8.2a |
| Preserve truthful locators | Use governed physical data; return unavailable when absent; never invent page ranges | Medium | 8.8.2a |
| Route every affected reader through the boundary | Cover `query_notebook`, `search_all_notebooks`, canonical-text `search_evidence`, document-block reads, and exact-chunk delivery; no affected path may retain the incompatible generic projection | High | 8.8.2b |
| Add schema-compatibility regression tests | Test both the certified older schema without page columns and a newer schema with them; FTS candidates and chunk identities must agree, while unavailable page locators remain explicit | High | 8.8.2b |
| Preserve the originating safe error | If a different storage failure occurs, retain a typed schema/retrieval reason and correlation ID rather than exposing only `sq-2:sparse`; coordinate with Module 8.8.7 | Medium | 8.8.2b, 8.8.7 |
| Prove corpus immutability | Record the certified DB SHA before and after tests; require no schema migration, reindex, FTS rebuild, embedding regeneration, or hash change | High | 8.8.2b |

**Module 8.8.2 acceptance gate:** the known certified database must complete
sparse retrieval and all other affected chunk-read paths without a missing-column
exception; FTS5 candidate identities must be preserved; absent page positions
must be reported truthfully; the opaque `sq-2:sparse` symptom must not conceal
the safe originating failure; and the certified corpus SHA must remain
byte-identical. Phase 8.8 cannot be marked verified while this gate is open.

**Module 8.8.3 — MCP Authorization Hardening**

| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Require transport-authenticated principal | stdio server subject and validated SSE/HTTP claims only | High | 8.8.1 |
| Apply central authorization to all 14 tools | Authorize notebook and subordinate identities before retrieval/delivery | High | 8.8.3a |
| Preserve non-disclosure | Unknown and unauthorized resources cannot become an existence oracle | High | 8.8.3b |
| Reject client policy overrides | Principal, store, model, reranker, generation, activation, and internal candidate K are server-owned | High | 8.8.3b |

**Module 8.8.4 — Existing 14-Tool Contract Completion**

| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Freeze route declaration per tool | Explicit retained V1, shared delivery, or certified V2 route; no accidental fallback | Medium | 8.8.2, 8.8.3 |
| Test valid requests | Every tool succeeds where its authorized data/capability exists | High | 8.8.4a |
| Test invalid/unavailable requests | Contract validation and scope-qualified unavailable results are distinct | High | 8.8.4a |
| Test provenance and transports | Identity, metadata, errors, and semantics agree across transports | High | 8.8.4b |

The existing tool inventory is fixed for this gate: `list_notebooks`,
`get_notebook_summary`, `get_timeline`, `get_source_insights`,
`search_all_notebooks`, `query_notebook`, `search_evidence`, `get_capabilities`,
`query_structured`, `get_document`, `get_document_chunk`, `get_asset`,
`get_image_analysis`, and `run_final_qa_v2`.

**Module 8.8.5 — Source Metadata and Filename Contract**

| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Define one authorized metadata envelope | Stable IDs plus nullable display name, original filename, title, MIME type, and content hash | Medium | 8.8.3 |
| Build one bounded resolver | Reused by retrieval, delivery, image, and citation responses; no path disclosure | High | 8.8.5a |
| Preserve identity semantics | Filename/title is presentation metadata, never authority or uniqueness | Medium | 8.8.5a |

**Module 8.8.6 — Capability Truthfulness**

| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Expand lifecycle semantics | Distinguish registered, implemented, configured, buildable, ready, active, exposed, available-for-scope, and certified | High | 8.8.1 |
| Bind effective identity | Return configuration/store identity appropriate to the caller and transport | Medium | 8.8.6a |
| Separate image capabilities | Asset delivery, OCR, caption, CLIP, and semantic visual search are independent states | Medium | 8.8.6a |

**Module 8.8.7 — Typed MCP Error Semantics**

| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Define safe error taxonomy | Input, unauthorized, not-found, unavailable, configuration, schema, retrieval, transport, timeout, server | Medium | 8.8.3 |
| Preserve originating safe code | Replace opaque `sq-2:sparse`-only failures with a typed layer/reason and correlation ID | Medium | 8.8.7a |
| Prevent leakage | No stack traces, SQL, filesystem paths, credentials, or unauthorized identity disclosure | High | 8.8.7a |

**Module 8.8.8 — V1/V2 Routing Contract**

| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Inventory and freeze routes | Retain V1 where contractually required; use V2 for V2 evidence/FinalQA; shared delivery remains explicit | Medium | 8.8.4 |
| Prevent historical fallback | Certified V2 calls cannot route to historical V1 store/model profiles | High | 8.8.8a |
| Verify frozen compatibility | Retained V1 behavior changes only for authorization and certified read compatibility | High | 8.8.8a |

**Module 8.8.9 — Transport Parity and Tunnel Certification**

| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Execute one four-transport matrix | HTTP, stdio, SSE, and existing tunnel use one effective composition | High | 8.8.1–8.8.8 |
| Compare semantic evidence | Store/model/activation IDs, principal, candidates, ranking, citations, metadata, provenance, capabilities, errors | High | 8.8.9a |
| Diagnose long FinalQA call | Correlate client/server start/completion, deadlines, process lifecycle, and MCP error envelope | High | 8.8.9a |

**Module 8.8.10 — Phase 8.6 Exposure Boundary**

| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Enforce evaluation-only selection | Production cannot federate the evaluation registry or accept filesystem paths | High | 8.8.1, 8.8.3 |
| Specify future promotion gate | Serving-role decision, allowlist, authorization, read compatibility, lifecycle, and fresh parity | Medium | 8.8.10a |
| Test isolation | Phase 8.5 production and Phase 8.6 evaluation identities never leak across scope | High | 8.8.10a |

**Module 8.8.11 — Structured Retrieval Contract**

| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Validate existing typed contract | Execute only against an existing authorized projection | Medium | 8.8.3, 8.8.6 |
| Return truthful unavailability | No projection/generation is an explicit capability result, not fake empty success | Medium | 8.8.11a |
| Keep activation out of scope | Tool registration does not activate structured retrieval as a product feature | Low | 8.8.11a |

**Module 8.8.12 — Dedicated Semantic Image Search (`search_images`)**

| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Define discovery request/result contracts | `notebook_id`, text `query`, `mode`, dynamic `requested_k`; stable image/source IDs, metadata, locators, retrieval paths, scores, provenance | High | 8.8.3, 8.8.5, 8.8.6 |
| Implement OCR mode | Search authorized OCR text and resolve results to image occurrences | High | 8.8.12a |
| Implement caption mode | Search authorized persisted Vision caption/observation text separately from OCR | High | 8.8.12a |
| Implement visual mode | Use the compatible CLIP text encoder and authorized persisted visual embeddings | High | 8.8.12a |
| Implement hybrid mode | Deterministic named-path RRF, identity deduplication, contribution reporting, and explicit partial/unavailable semantics | High | 8.8.12b–d |
| Connect discovery to delivery | `search_images` returns occurrence/asset identities; `get_asset` remains exact binary delivery | Medium | 8.8.12a |
| Test authorization first | No OCR/caption/vector path may disclose unauthorized images | High | 8.8.12b–e |

No scoring weights are selected merely to force a desired result. Any additional
fusion constant is frozen before evaluation and follows the existing RRF/
identity-deduplication architecture.

**Module 8.8.13 — End-to-End MCP Provenance**

| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Preserve full identity chain | Notebook → source → document → version → chunk/occurrence/asset → locator → representation/derivation → retrieval path | High | 8.8.4, 8.8.5, 8.8.12 |
| Validate citation/evidence resolution | Human metadata augments but never replaces canonical IDs | High | 8.8.13a |
| Compare transport provenance | Same semantic chain across HTTP, stdio, SSE, and tunnel | High | 8.8.9 |
...
|---|---|
| Runtime convergence | All four transports report the same certified configuration, production DB, model, activation, and authorization identities; mismatch startup fails closed |
| Existing tool completeness | Each of 14 tools has real valid, invalid-input, unauthorized, unavailable, provenance, typed-error, and transport evidence appropriate to its contract |
| Image discovery | `search_images` passes OCR, caption, CLIP visual, and hybrid queries; known-image, semantic, multilingual-where-supported, notebook scope, dynamic-k, no-result, unavailable-mode, provenance, and authorization cases |
| Retrieval | Retained V1 compatibility plus certified V2 sparse/dense/hybrid/multilingual behavior; internal pool remains 50 and public `requested_k` remains dynamic |
...
The completed [memory and artifact audit/cleanup](../../reports/operations/mnemo-memory-audit.md)
and [historical index and embedding cleanup](../../reports/operations/mnemo-historical-index-cleanup.md)
preserved all three protected stores, production configuration, models, and
certification state. Those audit reports remain the operational evidence; Phase
8.8 continues to treat these hashes as immutable entry/exit gates.

Failure is fail-closed: do not expose a divergent transport, mutate a protected
store, downgrade authorization, silently omit a requested modality, or mark the
phase verified. Roll back only through the governed runtime/configuration and
activation mechanisms.

Explicitly deferred: Qdrant production integration, SurrealDB production graph
integration, Phase 8.6 production promotion, arbitrary graph retrieval/graph UI,
large-scale vector infrastructure and Phase 13 scale work, unrelated storage
redesign, and certified V2 retrieval redesign.
...
**Module 10.1 — Notebook Enrichment on the Shared Job Facility**

| Task | Notes | Difficulty | Dependency |
|---|---|---|---|
| Extend `ProcessingJobStoreV1` handlers | Use the provider-neutral Phase 8.5 durable job contract; do not create a SurrealDB-only queue | High | Phase 9 |
| Implement slow path stages 9A–9D | NER, question gen, summaries, graph edges as typed job operations | High | 10.1a |
| Extend job priority policy | High-priority docs processed first within accepted admission/budget rules | Medium | 10.1a |
| Extend interruptible jobs | Reuse Phase 8.5 cancellation/resume/crash semantics | High | 10.1a |
...
| Store insight projections through the selected future adapter | SurrealDB remains an optional future target; this does not move canonical source or operational persistence out of the governed SQLite/filesystem boundaries | Medium | 10.3a |
...
Phase 2: Storage Layer                    (interfaces defined)
  ...
  │ Qdrant (optional V1)   │
  │ SurrealDB (partial)    │
  │ CompositeStorage       │ ← enabled-profile boundary
  └───────────┬────────────┘
              │
              ▼
Phase 3: Parser System
...
Phase 7: REST API          Phase 8: MCP Server
  ...
  WebSocket                  6 original MCP tools
  Auth Middleware
              │
              ▼
Phase 8.5: Advanced Retrieval + Multimodal Foundation
  Assets, Jobs, Completeness, Multimodal/Multilingual Evidence
              │
              ▼
Phase 8.6: Validated Evaluation Notebook (not production-exposed)
              │
              ▼
Phase 8.7: Expanded 14-Tool MCP / Client Capability
              │
              ▼
Phase 8.8: Certified Runtime + 15-Tool Contract + Phase 9 Gates
              │
              ▼
Phase 9: Web UI
...
Phase 7 (REST API) → Phase 8/8.5 certified services → Phase 8.8 verified
production/client boundary → Phase 9 UI MVP
```
...
| **Phase 8.5** | Advanced retrieval and multimodal foundations must preserve Phase 0–8 and freeze bounded API/capability contracts before UI work. |
| **Phase 8.6** | Format-diverse multilingual evaluation validates the architecture without promoting evaluation data into production. |
| **Phase 8.7** | Real MCP/client expansion exposes integration gaps that cannot be inferred from unit contracts alone. |
| **Phase 8.8** | Runtime convergence, all-tool correctness, semantic image discovery, mutable-workspace governance, and authenticated V2 chat are the final bounded prerequisites for UI production integration. |
| **Phase 9** | UI is the next implementation phase and begins only after Phase 8.8 issues a verified GO decision. It remains independent of Qdrant/SurrealDB selection. |
...
| **M2 — Storage Framework Live** | Filesystem/SQLite authority plus optional vector and partial graph adapters established | Canonical filesystem/SQLite operations pass; optional adapter contracts are independently testable and may be disabled by profile | Phase 2 |
...
| **M8.5 — V2 Certified** | Exact Full Multilingual V2 production composition certified | ADR-0076 evidence binds evaluation, durable BGE activation/rollback, HTTP/stdio/SSE, security, immutability, and lifecycle | Phase 8.5 |
| **M8.6 — Evaluation Notebook Validated** | Format-diverse multilingual notebook reproducible and isolated | 24/24 sources, 5,843 embeddings, 161 OCR/Vision/CLIP occurrences, canonical manifest, transport parity, evaluation-only boundary | Phase 8.6 |
| **M8.7 — MCP Surface Expanded** | Additive MCP capability and client milestone | 14 tools registered and real client paths exercised; discovered hardening gaps recorded without claiming full production convergence | Phase 8.7 |
| **M8.8 — MCP Production Contract Verified** | Certified composition reaches real tunnel with 15 governed tools | Runtime convergence, compatible readers, central authorization, metadata, capabilities/errors, image search, four-transport parity, mutable workspace and authenticated V2 chat gates pass | Phase 8.8 |
...
| **Integration** | Write/read canonical data through the enabled filesystem/SQLite profile; exercise optional Qdrant independently when selected |
| **Integration** | Delete/rollback across enabled adapters; disabled adapters remain untouched and do not block startup |
...
| Type | Tests |
|---|---|
| **Contract** | Model revisions, pair policy, RRF, internal K=50, dynamic public k, ContextBuilder, authorization, and evidence identities are frozen |
| **Evaluation** | Identity-bound production-parity evaluation and candidate/input audit pass against the exact 44-document store |
| **Integration** | Authenticated HTTP, MCP stdio, and MCP SSE V2 FinalQA pass with semantic parity |
| **Lifecycle** | Durable activation, restart reconstruction, rollback, reactivation, and WP-17 certification pass |
| **Safety** | Production corpus remains byte-identical; FinalQA writes only to its operational store |
...
| Type | Tests |
|---|---|
| **Corpus** | All 24 governed native-format sources ingest with deterministic structure-aware chunking |
| **Projection** | 5,843 FTS/BGE-M3 rows and 161 OCR/Vision/CLIP occurrence sets validate without orphans |
| **Manifest** | Canonical UTF-8 identity is stable for multilingual/Unicode content and registry rebinding |
| **Integration** | HTTP, stdio, and SSE agree on notebook/store/result identity |
| **Isolation** | Evaluation notebook cannot enter the production composition without an explicit promotion gate |
...
| Type | Tests |
|---|---|
| **Discovery** | Current server advertises exactly 14 implemented tool definitions |
| **Client** | Native and external clients exercise representative retrieval, delivery, analysis, and FinalQA calls |
| **Audit** | Production drift, authorization, reader, metadata, capability, error, and parity findings are preserved as Phase 8.8 inputs |
...
| Type | Tests |
|---|---|
| **Startup** | HTTP, stdio, SSE, and tunnel resolve one certified configuration/store/model/activation identity; divergence fails before exposure |
| **Tool matrix** | All 14 existing tools pass valid, invalid, unavailable, authorization, provenance, typed-error, and transport cases |
| **Image search** | `search_images` passes OCR, caption, CLIP, and hybrid discovery, dynamic-k, no-result, unavailable-modality, multilingual-where-supported, and authorization cases |
| **Security** | Anonymous/wrong principal and client principal/model/reranker/store/internal-K overrides fail without existence leakage |
...
| **P8.5 — Advanced V2** | Complete | Very High | Controlled | Exact production identity is certified; binding changes require recertification |
| **P8.6 — Evaluation** | Complete | High | Controlled | Evaluation-only identity and production isolation must remain explicit |
| **P8.7 — MCP Expansion** | Complete | High | High follow-up | Real clients revealed configuration, authorization, and reader-contract gaps |
| **P8.8 — MCP Convergence** | TBD during planning | Very High | **HIGH** | Tunnel identity convergence, 14 current tools plus planned `search_images` (15 only after implementation and verification), authorization, image retrieval, and Phase 9 data/chat boundaries |
...
- ☑ RRF fusion
- ☑ `CrossEncoderReranker` (**historical/evaluation-only V1** ADR-0042 pinned MS MARCO MiniLM L6; enhanced with ADR-0048 multi-source diversity ordering; not the certified V2 production reranker)
...
- ☑ Full Multilingual V2 candidate construction and retrieval
- ☑ BGE-M3 + FTS5 + RRF with internal reranker K=50
- ☑ BGE-reranker-v2-m3 governed activation/rollback
- ☑ Central V2 authorization and identity-bound evidence/provenance
- ☑ Separate mutable FinalQA operational store
- ☑ Production-parity evaluation and active HTTP/MCP verification
- ☑ WP-17 lifecycle certification under ADR-0076
- ☑ **[MILESTONE M8.5] Exact 44-document V2 production identity CERTIFIED**

### Phase 8.6 — Format-Diverse Multilingual Evaluation Notebook

- ☑ 24 governed PDF/HTML/DOCX/PPTX/XLSX/CSV/JSON/Markdown sources
- ☑ English/Hindi/Marathi ingestion and structure-aware chunking
- ☑ 5,843 FTS and BGE-M3 embeddings
- ☑ 161 image occurrences with OCR, Vision, and CLIP derivations
- ☑ Canonical UTF-8 manifest and registry validation
- ☑ HTTP/MCP stdio/MCP SSE identity parity
- ☑ Evaluation-only isolation from production
- ☑ **[MILESTONE M8.6] Evaluation notebook READY and validated**

### Phase 8.7 — MCP and Client Capability Expansion

- ☑ Expand server surface to 14 registered MCP tools
- ☑ Add native evidence, structured, delivery, capability, and FinalQA contracts
- ☑ Exercise representative calls from real clients
- ☑ Preserve discovered deployment/contract defects as Phase 8.8 inputs
- ☑ **[MILESTONE M8.7] Capability implementation complete; hardening explicitly pending**
...
- □ Add reusable authorized source metadata/filename envelope
- □ Make capability state scope-qualified and certification-aware
- □ Implement safe typed MCP error taxonomy and correlation
- □ Freeze explicit V1/V2 route declaration for every tool
- □ Certify four-transport parity and long-running tunnel FinalQA
- □ Enforce Phase 8.6 evaluation-only boundary
...
- □ Confirm Phase 8.8 GO evidence before implementation begins
...
*Architecture input: Mnemo Architecture Specification v2.0 (living current architecture)*  
